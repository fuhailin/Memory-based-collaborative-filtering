...
class IBCollaborativeFilter(object):
    ...
    def getRating(self, Train_data_matrix, userId, itemId, simility_matrix, knumber=20):
        ...
        averageOfUser = self.ItemMeanMatrix[itemId - 1]  # 获取userId 的平均值
        jiaquanAverage = (Train_data_matrix[test3] - self.ItemMeanMatrix[test3]).dot(
    ...
    def doEvaluate(self, testDataFrame, K):
        ...
        for row in testDataFrame.itertuples():
            prerating = self.getRating(self.train_data_matrix[row[1] - 1], row[1], row[2],
                                       self.SimilityMatrix[row[2] - 1], K)  # 基于训练集预测用户评分(用户数目<=K)
            self.lock.acquire()
