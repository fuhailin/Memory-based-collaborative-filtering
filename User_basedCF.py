...
class UBCollaborativeFilter(object):
    ...
    def getRating(self, Train_data_matrix, userId, simility_matrix, knumber=20):
        ...
        jiaquanAverage = (Train_data_matrix[recommendset] - self.UserMeanMatrix[recommendset]).dot(
            simility_matrix[recommendset])  # 计算每个用户的加权，预测
        self.RecallAndPrecision(recommendset, self.test_data_matrix[userId])
        if simSums == 0:
    ...
    def doEvaluate(self, testDataMatrix, K):
        ...
        for q, w in zip(a, b):
            ...
            prerating = self.getRating(self.train_data_matrix[:, w], q, self.SimilityMatrix[q],
